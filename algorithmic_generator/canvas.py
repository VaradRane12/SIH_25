...
def draw_path(draw, path_coords, color='black', width=2):
    """Draws a straight line connecting a sequence of coordinates."""
    if len(path_coords) > 1:
        draw.line(path_coords, fill=color, width=width)

def draw_bezier_curve(draw, start, control, end, width=2, color='black', segments=20):
    """
    ...
    for i in range(segments + 1):
        t = i / segments
        # Bézier curve formula for a single dimension (x or y)
        # P(t) = (1-t)^2 * P0 + 2*(1-t)*t * P1 + t^2 * P2
        x = ((1 - t)**2 * start[0]) + (2 * (1 - t) * t * control[0]) + (t**2 * end[0])
        y = ((1 - t)**2 * start[1]) + (2 * (1 - t) * t * control[1]) + (t**2 * end[1])
        curve_points.append((x, y))
    
    # Use the existing draw_path function to draw the curve
